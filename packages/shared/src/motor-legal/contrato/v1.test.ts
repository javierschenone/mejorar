/**
 * ADR-017 — el contrato es un artefacto único.
 *
 * `specs/contratos/motor-reglas-legales.ts` es la fuente normativa; la copia
 * de este paquete tiene que ser idéntica byte a byte. Este test es el control
 * que corre en la suite local; el pipeline corre además
 * `pnpm --filter @mejorar/shared verificar:contrato`, que es el mismo control
 * sin depender del runner.
 */
...
describe('ADR-017 — el contrato copiado no puede divergir de su fuente normativa', () => {
  ...
  it('la copia del paquete es idéntica byte a byte a specs/contratos/motor-reglas-legales.ts', () => {
  ...
  it('falla si el contenido se altera en una sola letra (demostración del control)', () => {
    ...
    expect(
      createHash('sha256').update(alterado).digest('hex'),
    ).not.toBe(huella(RUTA_FUENTE_NORMATIVA));
  });
